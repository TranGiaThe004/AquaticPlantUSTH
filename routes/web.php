...
Route::post('/forgot-password', [AuthWebController::class, 'sendResetLink'])->name('password.email');       // Gửi email reset link
...
Route::post('/login', [AuthWebController::class, 'loginUser'])->name('login.perform');     // POST /login -> xử lý login user (tạo session)
...
Route::post('/admin/login', [AuthWebController::class, 'loginAdmin'])->name('admin.login.perform');        // POST /admin/login -> xử lý login admin (session/guard admin)
...
Route::prefix('api')->group(function () { // Prefix /api cho các endpoint trả JSON (nhưng vẫn nằm trong web.php)
    ...
    // Public
    Route::post('/register', [AuthController::class, 'register']); // POST /api/register -> đăng ký (public)
    Route::post('/login',    [AuthController::class, 'login']);    // POST /api/login -> login (public)
